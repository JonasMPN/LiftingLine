...
def task1(debug=False):
    ...
    ll_results = {"r_centre": radii_centre, "u_induced": u_induced, "lift": lift,
                  "cl": cl, "a": axial_induction, "a_prime": a_prime, "aoa":np.rad2deg(effective_aoa),
...
def compare_ll_bem():
    ...
    axs[1].plot(results["r_centre"], results["a"])
    axs[1].plot(results["bem_results"]["r_centre"], results["bem_results"]["a"])

    axs[2].plot(results["r_centre"], results["a_prime"])
    axs[2].plot(results["bem_results"]["r_centre"], results["bem_results"]["a_prime"])

    axs[3].plot(results["r_centre"], results["aoa"])
    axs[3].plot(results["bem_results"]["r_centre"], results["bem_results"]["alpha"])
    helper.handle_axis(axs, x_label="radial position", grid=True, line_width=3,
                       font_size=12, y_label=["Cl", "a", "a_prime", "Angle of \nattack"])
    helper.handle_figure(fig, show=True, size=(6, 12))
    #plt.show()
